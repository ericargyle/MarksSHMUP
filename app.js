...
function resize(){ dpr=Math.min(window.devicePixelRatio||1,2); W=c.width=Math.floor(innerWidth*dpr); H=c.height=Math.floor(innerHeight*dpr); c.style.width='100vw'; c.style.height='100vh'; if(!player.x){ player.x=W*0.18; player.y=H*0.62; } }
...
function resetGame(){ bullets=[]; enemies=[]; particles=[]; boss=null; score=0; bombs=3; level=1; fireCd=0; spawnCd=0.4; terrainScroll=0; bossT=0; flash=0; shake=0; player={x:W*0.18,y:H*0.62,w:42*dpr,h:26*dpr,dragging:false,dx:0,dy:0,inv:1.2}; for(let i=0;i<6;i++) enemies.push(spawnEnemy(true)); updateHud(); }
...
function drawBullets(){ bullets.forEach(b=> b.laser ? rect(b.x,b.y,b.w,b.h,'#f6ff85') : circle(b.x,b.y,4*dpr,'#fff8df')); enemies.forEach(e=>{}); }
